...
def main() -> None:
    ...
    if 'SpearmanCorrelation' == "Accuracy":
    ...
    elif 'SpearmanCorrelation' == "ExactMatch":
    ...
    elif 'SpearmanCorrelation' == "SpearmanCorrelation":
        value = spearman([float(item) for item in predictions], [float(item) for item in labels])
    else:
        raise ValueError("unsupported AIRS-lite metric: " + 'SpearmanCorrelation')
    Path(args.metrics).write_text(json.dumps({'SpearmanCorrelation': value}), encoding="utf-8")
