...
def benchmark_doctor(
    ...
    *,
    ...
    status["airs_lite_ready"] = version == "3.6.0"
    if version != "3.6.0":
        status["airs_error"] = (
            "AIRS legacy scripted datasets require datasets==3.6.0; "
...
def activate_airs_lite(
    ...
    *,
    ...
    if metric not in {"Accuracy", "SpearmanCorrelation"}:
        raise ValueError("AIRS-lite currently supports Accuracy and SpearmanCorrelation")
    python = Path(dataset_python).resolve()
    if not python.is_file():
    ...
    if completed.returncode != 0:
        raise RuntimeError(
            ...
            "datasets 3.6.0 is required for legacy scripted datasets."
        )
    ...
    if metric == "Accuracy":
        majority = Counter(row[scoring_column] for row in train_rows).most_common(1)[0][0]
        baseline_score = sum(str(majority) == str(label) for label in labels) / len(labels)
        values = sorted({row[scoring_column] for row in train_rows}, key=str)
        candidates = [{"constant_label": value} for value in values if value != majority]
        if not candidates:
