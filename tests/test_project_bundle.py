...
def _full_draft(verdict: dict[str, object], source_ids: list[str]) -> PaperDraftSections:
    ...
    return PaperDraftSections(
        ...
        abstract=(
            "现有投资组合方法往往难以在保留高收益机会的同时控制退出风险。"
            "本研究在预先规定的研究问题、对照规则和前瞻盲测窗口下，检验赢家保护策略能否改善既定评价结果。"
            "配对比较显示，该策略在研究样本中改善了主要结果，同时保持了预设的保护条件。"
            "进一步分析考察了结果对观察窗口与评价口径的敏感性，主要方向在这些检查中保持一致。"
            "这一发现说明，面向极端收益机会的退出规则可能为收益保留与风险控制之间的权衡提供一种可检验方案。"
            "研究结果适用于当前任务、样本和市场窗口，仍需在更广泛的市场时期、资产类别与交易成本条件下继续检验。"
            "总体而言，该比较为后续前瞻研究提供了明确的经验起点，也说明评价退出策略时需要同时考察收益保持与下行风险。"
        ),
