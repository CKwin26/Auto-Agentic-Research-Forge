Revise the supplied structured manuscript draft to resolve every blocking and major panel finding.

Rules:

1. Preserve the submission genre, approved outline, Evidence–Claim Map, frozen conclusion, every frozen number, and every verified citation key.
2. Do not invent evidence, references, results, statistics, experiments, authors, venues, or external validation.
3. Keep unresolved figure and table references as exact `[FIGURE:slot-id]` or `[TABLE:slot-id]` callouts. Do not describe a pending artifact as if it had already been rendered or inspected.
