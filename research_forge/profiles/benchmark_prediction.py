"""Validation and reference evaluation for ``benchmark_prediction_v1``."""

from __future__ import annotations

import csv
from pathlib import Path
from typing import Any

from pydantic import ValidationError

from ..experiment_execution import ExperimentManifest, experiment_for_action
from ..workflow_domain import ResearchContractVersion
from .contracts import BenchmarkPredictionParameters, TabularMLParameters
from .tabular_ml import TabularPrediction, compute_metrics


def validate_benchmark_prediction_contract(
    contract: ResearchContractVersion,
    manifest: ExperimentManifest,
) -> list[str]:
    violations: list[str] = []
    try:
        parameters = BenchmarkPredictionParameters.model_validate(
            contract.profile_parameters
        )
    except ValidationError as exc:
        return [
            "profile_parameters."
            + ".".join(map(str, item["loc"]))
            + ": "
            + item["msg"]
            for item in exc.errors()
        ]
    if not contract.tasks or not contract.seeds:
        violations.append(
            "benchmark_prediction_v1 requires tasks and seeds"
        )
    if len(contract.hypotheses) != 1:
        violations.append(
            "benchmark_prediction_v1 requires exactly one primary hypothesis"
        )
    metric_names = [str(item.get("name") or "") for item in contract.metrics]
    if metric_names != parameters.metrics:
        violations.append(
            "contract metrics must exactly match frozen benchmark metrics"
        )
    for metric in contract.metrics:
        name = str(metric.get("name") or "")
        expected = (
            "lower_is_better" if name in {"rmse", "mae"} else "higher_is_better"
        )
        if metric.get("direction") != expected:
            violations.append(f"metric {name} direction must be {expected}")
        if not str(metric.get("formula") or "").strip():
            violations.append(f"metric {name} requires a formula")
        if not str(metric.get("denominator") or "").strip():
            violations.append(f"metric {name} requires a denominator")
    schema = contract.output_schema
    expected_schema = {
        "format": "json",
        "metric_field": parameters.primary_metric,
        "denominator_field": "denominator",
        "sample_id_field": "sample_ids",
        "analysis_rows_field": "analysis_rows",
        "record_layout": "summary_with_analysis_rows",
    }
    for field, expected in expected_schema.items():
        if schema.get(field) != expected:
            violations.append(f"output_schema.{field} must be {expected}")
    if contract.statistical_rules.get("method") != "paired_run_difference":
        violations.append(
            "benchmark_prediction_v1 method must be paired_run_difference"
        )
    if not isinstance(
        contract.statistical_rules.get("effect_threshold"), (int, float)
    ) or isinstance(
        contract.statistical_rules.get("effect_threshold"), bool
    ):
        violations.append("effect_threshold must be numeric")
    for arm_id, binding in (
        ("baseline", contract.baseline),
        ("treatment", contract.treatment),
    ):
        action_id = str(binding.get("action_id") or "")
        experiment_id = str(binding.get("experiment_id") or "")
        spec = experiment_for_action(manifest, action_id) if action_id else None
        if spec is None or spec.experiment_id != experiment_id:
            violations.append(
                f"{arm_id} binding does not match Experiment Manifest"
            )
            continue
        if spec.execution_backend != "isolated_candidate_evaluator":
            violations.append(f"{arm_id} must use isolated candidate/evaluator")
        if spec.network_access:
            violations.append(f"{arm_id} formal execution must be offline")
        if spec.required_inputs != [parameters.candidate_input_path]:
            violations.append(
                f"{arm_id} candidate input must exactly match frozen task input"
            )
        if spec.evaluator_required_inputs != [
            parameters.evaluator_target_path
        ]:
            violations.append(
                f"{arm_id} evaluator target must exactly match frozen target"
            )
        if set(spec.required_inputs).intersection(
            spec.evaluator_required_inputs
        ):
            violations.append(f"{arm_id} candidate can access formal targets")
        if not spec.candidate_code_paths or not spec.evaluator_code_paths:
            violations.append(
                f"{arm_id} must freeze candidate and evaluator code"
            )
        if spec.prediction_artifact_path != (
            parameters.prediction_artifact_path
        ):
            violations.append(
                f"{arm_id} prediction artifact path does not match contract"
            )
        if not spec.evaluator_command:
            violations.append(f"{arm_id} evaluator command is missing")
    return violations


def evaluate_csv_submission(
    parameters: BenchmarkPredictionParameters,
    *,
    submission_path: str | Path,
    target_path: str | Path,
) -> dict[str, Any]:
    """Independently recompute a frozen benchmark submission."""

    with Path(submission_path).open(
        "r", encoding="utf-8-sig", newline=""
    ) as handle:
        submissions = list(csv.DictReader(handle))
    with Path(target_path).open(
        "r", encoding="utf-8-sig", newline=""
    ) as handle:
        targets = list(csv.DictReader(handle))
    target_by_id = {
        str(row[parameters.id_field]): row[parameters.target_field]
        for row in targets
    }
    submission_ids = [
        str(row[parameters.id_field]) for row in submissions
    ]
    if (
        len(submission_ids) != len(set(submission_ids))
        or set(submission_ids) != set(target_by_id)
    ):
        raise ValueError(
            "submission IDs must exactly and uniquely match frozen targets"
        )
    predictions = [
        TabularPrediction(
            sample_id=sample_id,
            y_true=(
                str(target_by_id[sample_id])
                if parameters.task_type == "classification"
                else float(target_by_id[sample_id])
            ),
            y_pred=(
                str(row[parameters.prediction_field])
                if parameters.task_type == "classification"
                else float(row[parameters.prediction_field])
            ),
            y_score=(
                float(row[str(parameters.score_field)])
                if parameters.score_field
                else None
            ),
            fold=0,
        )
        for sample_id, row in zip(submission_ids, submissions)
    ]
    metric_spec = TabularMLParameters.model_validate(
        {
            "task_type": parameters.task_type,
            "dataset_path": "not-used.csv",
            "id_field": parameters.id_field,
            "target_field": parameters.target_field,
            "feature_fields": ["__benchmark_feature__"],
            "split_strategy": "fixed_split",
            "split_field": "__split__",
            "primary_metric": parameters.primary_metric,
            "metrics": parameters.metrics,
            "positive_label": parameters.positive_label,
            "baseline_estimator": {
                "import_path": "research_forge.profiles.tabular_ml:MajorityClassifier"
            },
            "treatment_estimator": {
                "import_path": "research_forge.profiles.tabular_ml:NearestCentroidClassifier"
            },
        }
    )
    metrics = compute_metrics(predictions, metric_spec)
    primary = metrics[parameters.primary_metric]
    result: dict[str, Any] = {
        "profile_id": "benchmark_prediction_v1",
        "metrics": metrics,
        "primary_metric": parameters.primary_metric,
        "primary_metric_value": primary,
        "denominator": len(predictions),
        "sample_ids": submission_ids,
        "analysis_rows": [
            {
                "pair_id": item.sample_id,
                "value": (
                    float(str(item.y_true) == str(item.y_pred))
                    if parameters.task_type == "classification"
                    else abs(float(item.y_true) - float(item.y_pred))
                ),
            }
            for item in predictions
        ],
    }
    result.update(metrics)
    return result


__all__ = [
    "evaluate_csv_submission",
    "validate_benchmark_prediction_contract",
]
